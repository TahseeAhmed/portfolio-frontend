...
import { ArrowDown, Download, Mail, ExternalLink, MapPin, GraduationCap } from 'lucide-react';
...
function TypewriterText({ texts }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (ref.current) ref.current.textContent = cur.slice(0, deleting ? j - 1 : j + 1);
      deleting ? j-- : j++;
      if (!deleting && j === cur.length) { deleting = true; setTimeout(tick, 1800); return; }
...
const timeline = [
  { year: '2022', title: 'Started CS Journey', desc: 'Enrolled at Sukkur IBA University, Computer Science.', icon: '🎓' },
  { year: '2023', title: 'First Full-Stack App', desc: 'Built my first MERN stack application end-to-end.', icon: '🚀' },
  { year: '2024', title: 'Open Source', desc: 'Started contributing to open source projects on GitHub.', icon: '🌍' },
  { year: '2025', title: 'Freelance Work', desc: 'Delivered 5+ client projects as a freelance developer.', icon: '💼' },
];
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-6xl w-full mx-auto grid md:grid-cols-2 gap-16 items-center relative z-10">
          ...
          <div className="space-y-7 animate-fade-up">
            ...
            <div className="space-y-2">
              <p className="font-mono text-gold text-sm tracking-widest uppercase">Hello, World!</p>
              <h1 className="font-display font-extrabold text-5xl md:text-6xl leading-[1.1] text-text">
                I'm <span className="gradient-gold">Tahseen</span><br />Ahmed
              </h1>
              <div className="flex items-center gap-2 text-2xl md:text-3xl font-display font-semibold text-muted mt-2 h-10">
                <TypewriterText texts={['Full Stack Dev', 'MERN Specialist', 'Problem Solver', 'CS Student']} />
                <span className="text-gold animate-pulse">|</span>
              </div>
            </div>

            <p className="text-muted font-body text-base leading-relaxed max-w-md">
              {profile?.subtitle || 'Building scalable web applications with clean code & creative thinking.'} Studying at{' '}
              <span className="text-teal font-medium">Sukkur IBA University</span>.
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="flex items-center gap-1.5 text-xs text-muted border border-slate/50 rounded-lg px-3 py-1.5 bg-surface">
                <GraduationCap size={11} className="text-teal" /> Sukkur IBA University
              </span>
            </div>
          ...
          <div className="flex justify-center md:justify-end">
            <div className="relative">
              <div className="w-72 h-72 md:w-80 md:h-80 rounded-3xl border border-slate/60 overflow-hidden bg-surface shadow-2xl shadow-navy">
                {profile?.profileImage?.url ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center gap-3 bg-gradient-to-br from-surface to-navy">
                    ...
                    <p className="text-muted text-xs font-mono">No photo uploaded yet</p>
                    <Link to="/admin" className="text-xs text-gold hover:underline">Upload in Admin →</Link>
                  </div>
                )}
              </div>
              {/* Corner accents */}
              ...
              <div className="absolute -left-10 top-10 bg-surface border border-slate/60 rounded-xl px-3.5 py-2.5 shadow-xl">
                ...
                <p className="text-2xl font-display font-bold gradient-gold">{projects.length}+</p>
              </div>
              <div className="absolute -right-10 bottom-10 bg-surface border border-slate/60 rounded-xl px-3.5 py-2.5 shadow-xl">
                ...
                <p className="text-2xl font-display font-bold gradient-teal">{skills.length}+</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-14 px-6 border-y border-slate/30">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            { label: 'Projects Built',  value: `${projects.length}+`, color: 'gradient-gold' },
            { label: 'Technologies',    value: `${skills.length}+`,   color: 'gradient-teal' },
            { label: 'Institute',       value: 'SIBA',                color: 'gradient-gold' },
            { label: 'Status',          value: 'Hiring',              color: 'gradient-teal' },
          ].map((s, i) => (
            <div key={i}>
              <p className={`font-display font-extrabold text-4xl ${s.color}`}>{s.value}</p>
              <p className="text-muted text-xs font-mono mt-1 tracking-wide uppercase">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-14">
            <p className="font-mono text-gold text-xs tracking-widest uppercase mb-2">Career Path</p>
            <h2 className="font-display font-bold text-4xl text-text">My Journey</h2>
          </div>
          <div className="relative">
                  ...
                  <div className="flex-1 pl-14 md:pl-0">
                    {i % 2 === 0 && (
                      <div className="bg-surface border border-slate/50 rounded-2xl p-5 card-hover">
                        ...
                        <h3 className="font-display font-semibold text-text mt-1">{item.title}</h3>
                        <p className="text-muted text-sm font-body mt-1 leading-relaxed">{item.desc}</p>
                      </div>
                    )}
                  </div>
        ...
        <section className="py-20 px-6 bg-surface/30">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-end justify-between mb-12">
              <div>
                <p className="font-mono text-gold text-xs tracking-widest uppercase mb-2">Selected Work</p>
                <h2 className="font-display font-bold text-4xl text-text">Featured Projects</h2>
              </div>
              <Link to="/projects" className="text-muted text-sm hover:text-gold transition-colors font-mono hidden md:block">
                View all →
              </Link>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
              {projects.filter(p => p.featured).slice(0, 2).map((project, i) => (
                <div key={project._id || i} className="bg-navy border border-slate/50 rounded-2xl overflow-hidden card-hover group">
                  <div className="h-44 bg-gradient-to-br from-slate/40 to-navy-light flex items-center justify-center relative">
                    {project.image?.url ? (
                      ...
                      <span className="font-display font-bold text-5xl text-slate/40">{project.title.slice(0, 2)}</span>
                    )}
                  ...
                  <div className="p-5">
                    <h3 className="font-display font-bold text-lg text-text group-hover:text-gold transition-colors">{project.title}</h3>
                    <p className="text-muted text-sm font-body mt-1 leading-relaxed line-clamp-2">{project.description}</p>
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {project.tech?.slice(0, 4).map(t => (
                        <span key={t} className="text-[10px] bg-slate/30 text-muted px-2 py-0.5 rounded font-mono">{t}</span>
                      ))}
                    </div>
                    <div className="flex gap-4 mt-4 pt-3 border-t border-slate/30">
                      {project.github && <a href={project.github} target="_blank" rel="noreferrer" className="text-muted text-xs hover:text-text transition-colors font-mono">GitHub ↗</a>}
                      {project.live && project.live !== '#' && <a href={project.live} target="_blank" rel="noreferrer" className="text-muted text-xs hover:text-teal transition-colors font-mono">Live Demo ↗</a>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center mt-8 md:hidden">
              <Link to="/projects" className="text-muted text-sm hover:text-gold transition-colors font-mono">View all projects →</Link>
            </div>
          </div>
        </section>
      )}
